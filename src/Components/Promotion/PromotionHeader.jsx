...
const PromotionHeader = ({language,sectionsData,itemsData}) => {
  ...
  const items = itemsData[language];
    return (
      ...
      <div className="flex text-white pb-[40%]  bg-red flex-col items-center justify-center">
      ...
      {sections.map((section, index) => (
        <div key={index} className="cursor-pointer whitespace-nowrap   shadow-md bg-white">
          <h3 className="bg-red text-white text-center p-2 ">{section}</h3>
          <div className="space-y-2 mt-2 pb-2 px-4 md:px-12">
            {items.map((item, idx) => (
              <div key={idx} className={`flex flex-col ${idx===3?"whitespace-normal ":""} items-center`}>
