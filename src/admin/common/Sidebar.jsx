...
import logo from "../../assets/tab8.png"
...
const Sidebar = () => {
  ...
  const SIDEBAR_ITEMS = [
	...
	},
	{
		...
		submenu: [
			{ name: "Add Banner", href: "/dashboard/add-slider"},
			{ name: "Banner List", href: "/dashboard/slider-list"},
		],
	},
	{
		name: "Games",
		...
		submenu: [
			{ name: "Add Game", href: "/dashboard/add-game"},
			{ name: "Game List", href: "/dashboard/all-games"},
		],
	},
	{
		name: "All User",
    ...
    { name: "Game Control", icon: PiGameControllerDuotone, color: "#EC4899", href: "/users",submenu: [
		{ name: "Unibet Hub Game", href: "/admin-dashboard/unibethub-all-games" },
		{ name: "Game Logs", href: "/admin-dashboard/unibethub-game-logs"},
	],},
    ...
    { name: "Settings", icon: VscSettingsGear, color: "#EC4899", href: "/admin-dashboard/settings",
	// 	submenu: [
	...
	return (
		<motion.div
			className={`no-scrollbar hidden xl:block  relative z-10 transition-all font-bai duration-300 h-screen ease-in-out flex-shrink-0 no-scrollbar ${
				isSidebarOpen ? "w-64" : "w-20"
			}`}
			animate={{ width: isSidebarOpen ? "18%" : 80 }}
		>
			<div className='no-scrollbar  h-full overflow-y-auto bg-[#071251] custom-scrollbar backdrop-blur-md px-[10px] py-4 flex flex-col '>
				<div className={isSidebarOpen ? "flex justify-between items-center":"flex justify-center items-center"}>
					...
					<div className='py-4 '>
						<img className="w-[90px] mb-[6px]" src={logo} alt="" />
                        {
