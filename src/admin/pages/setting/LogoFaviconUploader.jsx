...
export default function LogoFaviconUploader() {
  ...
  const imageBaseUrl = "https://api.wingobd.com/images/";

  // Fetch current logo and favicon from the backend
  useEffect(() => {
    const fetchLogoFavicon = async () => {
      try {
        const response = await axios.get(`https://api.wingobd.com/admin/get-logo-favicon`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://api.wingobd.com/admin/upload-logo-favicon`, formData, {
