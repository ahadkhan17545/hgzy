...
// ----------api-model------------------
admin_route.post("/add-api-game", uploadimage.single("image"), async (req, res) => {
  try {
    const { category, name, liveUrl } = req.body;

    // Validation
    if (!category || !liveUrl || !req.file) {
      return res.status(400).json({ message: "All required fields must be filled" });
    }

    // Save game to the database
    const newGame = new Apimodel({
      category,
      name,
      liveUrl,
      imagePath: req.file.filename, // Save the image filename
    });
...
// @desc    Get all games
// @access  Public
admin_route.get("/all-api-games", async (req, res) => {
  try {
    const games = await Apimodel.find();
    res.json(games);
  } catch (error) {
